...
describe('Variables', () => {
  ...
  describe('fallback', () => {
    ...
    it('should throw an error if ${self} syntax fail to populate and no fallback provided', () => {
      serverless.variables.service.custom = {
        settings: '${self:nonExistent}',
      };
      return serverless.variables.populateService().should.be.rejectedWith(
    ...
    it('should throw an error if ${opt} syntax fail to populate and no fallback provided', () => {
      serverless.variables.service.custom = {
        settings: '${opt:nonExistent}',
      };
      return serverless.variables.populateService().should.be.rejectedWith(
        serverless.classes.Error
      );
    ...
    it('should throw an error if ${env} syntax fail to populate and no fallback provided', () => {
      serverless.variables.service.custom = {
        settings: '${env:nonExistent}',
      };
      return serverless.variables.populateService().should.be.rejectedWith(
        serverless.classes.Error
      );
    ...
    describe('file syntax', () => {
      ...
      it('should throw an error if file does not exist and no fallback provided', () => {
        serverless.variables.service.custom = {
          settings: '${file(~/config.yml):xyz}',
        };
      ...
      it('should throw an error if file exists, key not found and no fallback provided', () => {
        serverless.variables.service.custom = {
          settings: '${file(~/config.yml):xyz}',
        };
...
`;
          ...
          () => {
            ...
            return serverless.variables.populateObject(service.custom)
              .should.be.rejectedWith(serverless.classes.Error,
                'Variable "notAValue" is not found {"value":"a value"}');
          });
      });
    });
  });
  ...
  describe('#populateProperty()', () => {
    ...
    it('should warn if an SSM parameter does not exist and fallback given', () => {
      const options = {
        stage: 'prod',
        region: 'us-east-1',
      };
      ...
      const property = `\${ssm:${param}, 'fallback'}`;
    ...
    it('should NOT warn if an SSM parameter does not exist and no fallback given', () => {
      const options = {
        stage: 'prod',
        region: 'us-east-1',
      };
  ...
  describe('#getValueFromFile()', () => {
    ...
    it('should throw an error if non existing file and the second argument is true', () => {
      const tmpDirPath = testUtils.getTmpDirPath();
      serverless.config.update({ servicePath: tmpDirPath });
      const realpathSync = sinon.spy(fse, 'realpathSync');
      const existsSync = sinon.spy(fse, 'existsSync');
      return serverless.variables.getValueFromFile('file(./non-existing.yml)').should.be.rejected
        .then((valueToPopulate) => {
          expect(realpathSync).to.not.have.been.called;
          expect(existsSync).to.have.been.calledOnce;
          expect(valueToPopulate).to.be.instanceOf(serverless.classes.Error);
        })
        .finally(() => {
          realpathSync.restore();
          existsSync.restore();
  ...
  describe('#getValueFromSsm()', () => {
    ...
    it('should reject if SSM parameter returns any error', () => {
      const error = new serverless.classes.Error(`Parameter ${param} not found.`);
      const requestStub = sinon.stub(awsProvider, 'request', () => BbPromise.reject(error));
      return serverless.variables.getValueFromSsm(`ssm:${param}`)
        .should.be.rejectedWith(serverless.classes.Error, [
