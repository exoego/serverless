...
class Variables {
  ...
  getValueFromEnv(variableString) { // eslint-disable-line class-methods-use-this
    ...
    if (_.isUndefined(valueToPopulate)) {
      const errorMessage = [
        `Environment variable "${variableString}" is not provided`,
      ].join(' ');
  ...
  getValueFromOptions(variableString) {
    ...
    if (_.isUndefined(valueToPopulate)) {
      const errorMessage = [
        `Option "${variableString}" is not provided`,
      ].join(' ');
      return BbPromise.reject(new this.serverless.classes.Error(errorMessage));
    }
  ...
  getValueFromFile(variableString) {
    ...
    if (!this.serverless.utils.fileExistsSync(referencedFileFullPath)) {
      const errorMessage = [
        `Referenced file "${referencedFileRelativePath}" does not exist.`,
      ].join(' ');
      return BbPromise.reject(new this.serverless.classes.Error(errorMessage));
    }
  ...
  getDeeperValue(deepProperties, valueToPopulate) {
    ...
    }, valueToPopulate).then(reducedValue => {
      if (_.isUndefined(reducedValue)) {
        const errorMessage = [
          `Variable "${deepProperties}" is not found ${JSON.stringify(valueToPopulate)}`,
        ].join(' ');
        return BbPromise.reject(new this.serverless.classes.Error(errorMessage));
      }
      return BbPromise.resolve(reducedValue);
