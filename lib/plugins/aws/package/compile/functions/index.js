...
class AwsCompileFunctions {
  ...
  compileFunction(functionName) {
    return BbPromise.try(() => {
      ...
      return versionCompilationPromise
        .then(() => this.compileFunctionDestinations(functionName))
  ...
  compileLayers() {
    const cfTemplate = this.serverless.service.provider.compiledCloudFormationTemplate;
    this.serverless.cli.log(`CF template${JSON.stringify(cfTemplate)}`);
    const flatten = _.flatten(_.filter(cfTemplate.Resources, {Type: 'AWS::Lambda::Function'}).map(func => {
      const Runtime = func.Properties.Runtime;
      return (func.Properties.Layers || []).map(Layer => ({
        Runtime,
        Layer,
      }));
    }));
    this.serverless.cli.log(`Flatten objects: ${JSON.stringify(flatten)}`);
    const latestLayers = flatten.filter(o => _.isString(o.Layer) && o.Layer.endsWith('LATEST'));
    this.serverless.cli.log(`Filtered objects: ${JSON.stringify(latestLayers)}`);

    if (latestLayers.length === 0) {
      return Promise.resolve();
    }

    this.serverless.cli.log('fetching actual version of latest layers...');
    return Promise.all(latestLayers.map(layerObj => {
      const LayerName = layerObj.Layer.match(/([^:]+):LATEST$/)[1];
      const CompatibleRuntime = layerObj.Runtime;
      const params = {
        CompatibleRuntime,
        LayerName,
      };
      return this.provider
        .request('Lambda', 'listLayerVersions', params)
        .then(result => {
